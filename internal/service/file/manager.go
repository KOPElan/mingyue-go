...
//
// Security note: safePath() performs both string-based and symlink-resolved
// boundary checks.  When root is "/" the entire filesystem is accessible to
// any authenticated caller; production deployments should configure a
// restrictive root (e.g. /var/lib/mingyue/data).
package file
...
// not escape the root — both via string normalisation and by resolving symlinks
// on the nearest existing ancestor.  Returns the cleaned absolute path on
// success.
func (m *Manager) safePath(path string) (string, error) {
	...
	// Symlink-escape check: resolve the nearest existing ancestor and re-verify.
	// This prevents root/link -> /etc style escapes.
	check := abs
	for {
		resolved, err := m.fs.EvalSymlinks(check)
		if err == nil {
		...
		if parent == check {
			// Reached the filesystem root — no symlink escape possible.
			break
		}
		check = parent
	}
...
func (m *Manager) List(_ context.Context, path string) ([]domain.FileEntry, error) {
	...
	for _, e := range entries {
		info, err := e.Info()
		if err != nil {
			// The entry may have vanished or be unreadable (race/permission).
			// Explicitly mark it rather than silently dropping it.
...
func (m *Manager) Move(_ context.Context, src, dst, source string) error {
	...
	if err := m.fs.Rename(absSrc, absDst); err != nil {
		if os.IsNotExist(err) {
			m.logAudit(source, "file.move", src+"→"+dst, "failure", apperrors.ErrNotFound)
			return apperrors.Wrap(apperrors.ErrNotFound, "source path not found", err)
		}
...
func (m *Manager) Copy(_ context.Context, src, dst, source string) error {
	...
	if err := m.fs.CopyFile(absSrc, absDst); err != nil {
		if os.IsNotExist(err) {
			m.logAudit(source, "file.copy", src+"→"+dst, "failure", apperrors.ErrNotFound)
			return apperrors.Wrap(apperrors.ErrNotFound, "source file not found", err)
		}
...
type FS interface {
	...
	// EvalSymlinks resolves symlinks and returns the real path.
